#!/usr/bin/env python3
"""Fresh top/bottom-electrode Eb optimization with evaporated-SiO2 G."""
...
ADFD_ROOT = ARTIFACT_ROOT / "combined_adfd_Eb_evaporated_run050_precheck"
ADFD_RESULT = ADFD_ROOT / "tairte4_flake_combined_adfd.json"
OPTIMIZATION_ROOT = ARTIFACT_ROOT / "run050_Eb_evaporated_fresh_current_max"
RESULTS = HERE / "results"
STATE = HERE / "PIPELINE_STATE.json"
LOCK = Path("/tmp/seunghyun_run050_top_bottom_evaporated_Eb.lock")
...
def write_state(stage: str, **extra: object) -> None:
    payload = {
        ...
        "electrical_terminals": {"bottom": 0, "top": 1},
...
def main() -> int:
    ...
    try:
        ...
        raise RuntimeError("Run050 pipeline is already active") from error

    if not BASE_FSP.is_file() or sha256(BASE_FSP) != BASE_SHA256:
        raise RuntimeError("immutable top/bottom uniform-rho0.5 template FSP changed")
    ...
    if OPTIMIZATION_ROOT.exists() and any(OPTIMIZATION_ROOT.iterdir()):
        raise RuntimeError("Run050 raw output is non-empty; refusing non-fresh start")
    if RESULTS.exists() and any(RESULTS.iterdir()):
        raise RuntimeError("Run050 published output is non-empty; refusing non-fresh start")

    if ADFD_ROOT.exists():
        if not ADFD_RESULT.is_file() or not json.loads(ADFD_RESULT.read_text()).get("passed"):
            raise RuntimeError("existing evaporated-interface Eb AD-FD output is incomplete")
        ...
        if contract.get("TaIrTe4_SiO2_scenario") != INTERFACE_SCENARIO:
            raise RuntimeError("existing AD-FD interface scenario does not match Run050")
    else:
        run_checked(
            [
                str(PYTHON), "-m",
            ...
            stage="combined_adfd_Eb_evaporated",
        )
    precheck = json.loads(ADFD_RESULT.read_text())
    contract = precheck.get("thermal_interface_contract", {})
    if (
        not precheck.get("passed")
        or contract.get("TaIrTe4_SiO2_scenario") != INTERFACE_SCENARIO
        or float(contract.get("G_TaIrTe4_SiO2_W_m2K", 0.0)) != INTERFACE_G_W_M2K
    ):
        raise RuntimeError("Run050 evaporated-interface Eb AD-FD failed")
    ...
    run_checked(
        ...
        stage="optimization_Eb_evaporated",
    )
    final_result = RESULTS / "FINAL_RESULT.json"
    if not final_result.is_file():
        raise RuntimeError("Run050 returned without FINAL_RESULT.json")
