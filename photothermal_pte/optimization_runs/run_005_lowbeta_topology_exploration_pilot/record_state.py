...
def save_design_plots(tag: str, metrics: dict, arrays: dict, data) -> list[Path]:
    ...
    axes[1].scatter(
        ["solid", "void"],
        [metrics["solid_constraint_cap"], metrics["void_constraint_cap"]],
        color="black", marker="_", s=500, linewidths=2, label="phase cap",
    )
...
def save_history_outputs(summary: dict[str, object]) -> None:
    ...
    axes[0, 2].semilogy(x, [row["solid_constraint_cap"] for row in history], "k--", label="solid cap")
    axes[0, 2].semilogy(x, [row["void_constraint_cap"] for row in history], "k:", label="void cap")
    axes[0, 2].legend()
...
def record(
    ...
    row = {
        ...
        "solid_constraint_cap": metrics["solid_constraint_cap"],
        "void_constraint_cap": metrics["void_constraint_cap"],
    ...
    report = (
        ...
        f"with caps `{metrics['solid_constraint_cap']:.6e}` / "
        f"`{metrics['void_constraint_cap']:.6e}`.\n\n"
