#!/usr/bin/env python3
"""Launch fresh trust-region Run055 on an explicitly selected GPU."""
...
RAW = ARTIFACT_ROOT / "run055_bounded_official_dfm_exact_repair_Ea_evaporated_v1"
...
def main() -> int:
    gpu = int(os.environ.get("RUN055_GPU", "1"))
    ...
    environment.update(
        {
            ...
            "XDG_CONFIG_HOME": f"/tmp/seunghyun_lumerical_run055_gpu{gpu}",
            "MPLCONFIGDIR": f"/tmp/seunghyun_matplotlib_run055_gpu{gpu}",
        }
    )
    command = [
        sys.executable,
    ...
    if os.environ.get("RUN055_RESUME", "0") == "1":
