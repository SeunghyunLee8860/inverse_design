#!/usr/bin/env python3
"""Launch fresh +45-degree thermally-grown-SiO2 E||a-only Run063."""
...
RAW = ARTIFACT_ROOT / "run063_diagonal45_single_Ea_thermally_grown_v1"
...
def main() -> int:
    ...
    environment.update(
        {
            ...
            "LUMERICAL_GPU_ENGINE_LOCK": f"/tmp/seunghyun_lumerical_run063_gpu{gpu}.lock",
            "XDG_CONFIG_HOME": f"/tmp/seunghyun_lumerical_run063_gpu{gpu}",
            "MPLCONFIGDIR": f"/tmp/seunghyun_matplotlib_run063_gpu{gpu}",
        }
    )
    command = [
        sys.executable,
    ...
    if os.environ.get("RUN063_RESUME", "0") == "1":
