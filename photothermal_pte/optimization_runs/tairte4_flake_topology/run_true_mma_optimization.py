...
def discard_regenerable_evaluation_solver_files(output: Path) -> list[dict[str, object]]:
    ...
    Density, objective-gradient NPZ, JSON, logs, and plots are deliberately
    excluded.  The removed files are regenerated by the next licensed solve.
    """
    ...
    for path in sorted(output.rglob("*")):
        if not path.is_file():
            continue
        if path.suffix.lower() != ".fsp" and not path.name.endswith("_output.h5"):
            continue
        discarded.append(
            {
                "path": str(path),
    ...
    *,
    ...
    while True:
        ...
        if completed.returncode == 0 and result is not None and result.get("passed"):
            discarded = discard_regenerable_evaluation_solver_files(output)
            if discarded:
                retention = result.setdefault("large_project_retention", {})
                retention.setdefault("discarded_solver_outputs", []).extend(discarded)
                retention["solver_output_h5_retained"] = False
        ...
        if discarded:
            emit(
                events,
                "failed_evaluation_solver_outputs_discarded",
                output=str(output),
                discarded=discarded,
            )
        if not retryable:
