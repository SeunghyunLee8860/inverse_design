...
def _long_cpml_phasor_scene(*, total_steps: int = 70):
    import fdtdx
    import jax
    ...
    config = config.aset("time", total_steps * float(config.time_step_duration))
    ...
    detector = fdtdx.PhasorDetector(
        ...
        switch=fdtdx.OnOffSwitch(
            start_time=24 * float(config.time_step_duration)
        ),
...
def _relative_l2(observed, expected) -> float:
    observed_array = np.asarray(observed, dtype=np.float64)
    expected_array = np.asarray(expected, dtype=np.float64)
    numerator = np.linalg.norm(observed_array - expected_array)
    denominator = np.linalg.norm(expected_array)
    return float(numerator / denominator)


def test_long_partial_slices_stabilize_phasor_c3_gradient() -> None:
    ...
    def sliced_loss(value, steps_per_slice):
    ...
    unsliced_value, unsliced_gradient = jax.value_and_grad(unsliced_loss)(c3)
    sliced_results = {
        steps: jax.value_and_grad(lambda value: sliced_loss(value, steps))(c3)
        for steps in (7, 16)
    }
    direct_gradient_array = np.asarray(direct_gradient)
    unsliced_error = _relative_l2(unsliced_gradient, direct_gradient_array)
    sliced_errors = {
        steps: _relative_l2(gradient, direct_gradient_array)
        for steps, (_, gradient) in sliced_results.items()
    }

    assert float(unsliced_value) == pytest.approx(
        float(direct_value), rel=1.0e-6, abs=1.0e-10
    )
    assert math.isfinite(unsliced_error)
    for steps, (value, gradient) in sliced_results.items():
        assert float(value) == pytest.approx(
            float(direct_value), rel=1.0e-6, abs=1.0e-10
        )
        np.testing.assert_allclose(
            np.asarray(gradient),
            direct_gradient_array,
            rtol=1.0e-3,
            atol=3.0e-10,
        )
        assert sliced_errors[steps] <= unsliced_error
    assert np.count_nonzero(direct_gradient_array) > 0
