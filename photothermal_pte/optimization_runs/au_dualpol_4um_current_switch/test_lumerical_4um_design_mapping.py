...
def test_lumerical_cell_dfm_constraint_gradient_reaches_nodal_latent() -> None:
    ...
    direction /= np.max(np.abs(direction))
    beta = 4.0
    values, gradients, fields = smooth_lumerical_500nm_constraints(latent, beta)
    step = 2.5e-4
    plus = smooth_lumerical_500nm_constraints(
        latent + step * direction, beta
    )[0]
    minus = smooth_lumerical_500nm_constraints(
        latent - step * direction, beta
    )[0]
...
def test_binary_candidate_is_cell_based_and_requires_exact_au_reevaluation() -> None:
    ...
    assert audit["solid_pass"] is False
