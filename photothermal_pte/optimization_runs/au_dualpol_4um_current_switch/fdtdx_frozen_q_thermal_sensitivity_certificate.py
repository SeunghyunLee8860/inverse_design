#!/usr/bin/env python3
"""Certify integrity of frozen-Q thermal physical-scenario sensitivities."""

from __future__ import annotations

import argparse
from datetime import datetime, timezone
import json
import os
from pathlib import Path
import subprocess
import traceback
from typing import Any, Mapping

import numpy as np

from photothermal_pte.optimization_runs.au_dualpol_4um_current_switch.fdtdx_frozen_q_thermal_sensitivity_case import (
    REPORT_NAME,
    SCENARIO_PARAMETERS,
    SELECTED_DOMAIN,
    STATUS_READY as CASE_STATUS_READY,
    THERMAL_XY_REFINEMENT_FACTOR,
    THERMAL_Z_REFINEMENT_FACTOR,
    VERSION as CASE_VERSION,
    audit_prior_domain_certificate,
    sha256,
)
from photothermal_pte.optimization_runs.au_dualpol_4um_current_switch.multiphysics_4um import (
    G_AU_TA_W_M2K,
    G_TA_SIO2_W_M2K,
    K_TA_XYZ_W_MK,
    TOP_AIR_CONVECTION_W_M2K,
    thermal_edges,
)


VERSION = "fdtdx-frozen-q-thermal-sensitivity-certificate-v1"
STATUS_READY = (
    "VALIDATED_DIAGNOSTIC_FDTDX_FROZEN_Q_THERMAL_SCENARIO_"
    "SENSITIVITY_WITH_PHYSICAL_BOUNDS_BLOCKED"
)
STATUS_BLOCKED = "BLOCKED_FDTDX_FROZEN_Q_THERMAL_SCENARIO_SENSITIVITY"
STATUS_EXCEPTION = "BLOCKED_FDTDX_FROZEN_Q_THERMAL_SENSITIVITY_CERTIFICATE_EXCEPTION"
CERTIFICATE_NAME = "FDTDX_FROZEN_Q_THERMAL_SENSITIVITY_CERTIFICATE.json"
POLARIZATIONS = ("Ea", "Eb")
BASE_FIELD_NAMES = (
    "ta_temperature_rise_K",
    "ta_gradient_x_K_m",
    "ta_gradient_y_K_m",
    "ta_x_centers_m",
    "ta_y_centers_m",
    "source_power_xy_W",
)
SOURCE_NRMSE_LIMIT = 5.0e-12
BASE_COORDINATE_ATOL_M = 2.0e-18


def _all_true(values: Mapping[str, Any]) -> bool:
    return bool(values) and all(value is True for value in values.values())


def _relative(left: float, right: float) -> float:
    return abs(left - right) / max(abs(left), abs(right), np.finfo(float).tiny)


def _nrmse(left: np.ndarray, right: np.ndarray) -> float:
    return float(
        np.linalg.norm(np.asarray(right) - np.asarray(left))
        / max(float(np.linalg.norm(right)), np.finfo(float).tiny)
    )


def _git(repository: Path, *arguments: str) -> str:
    return subprocess.run(
        ("git", "-C", str(repository), *arguments),
        check=True,
        capture_output=True,
        text=True,
    ).stdout.strip()


def _atomic_json(path: Path, payload: Mapping[str, Any]) -> None:
    temporary = path.with_suffix(".tmp")
    temporary.write_text(
        json.dumps(payload, indent=2, sort_keys=True, allow_nan=False) + "\n",
        encoding="utf-8",
    )
    os.replace(temporary, path)


def expected_model_parameters(scenario_id: str) -> dict[str, Any]:
    result: dict[str, Any] = {
        "far_xy_boundary": "ambient_dirichlet",
        "top_air_convection_W_m2K": TOP_AIR_CONVECTION_W_M2K,
        "g_ta_sio2_W_m2K": G_TA_SIO2_W_M2K,
        "g_au_ta_W_m2K": G_AU_TA_W_M2K,
        "ta_kappa_xyz_W_mK": [float(value) for value in K_TA_XYZ_W_MK],
    }
    result.update(SCENARIO_PARAMETERS[scenario_id])
    return result


def _raw_snapshot(path: Path) -> tuple[dict[str, np.ndarray], dict[str, bool]]:
    required = {
        *BASE_FIELD_NAMES,
        "ta_temperature_native_K",
        "ta_x_native_centers_m",
        "ta_y_native_centers_m",
        "source_power_native_xy_W",
        "thermal_x_native_centers_m",
        "thermal_y_native_centers_m",
        "thermal_z_centers_m",
        "center_temperature_rise_K",
    }
    with np.load(path, allow_pickle=False) as archive:
        exact = set(archive.files) == required
        snapshot = {name: np.asarray(archive[name]) for name in required}
    checks = {
        "arrays_declared_exactly": exact,
        "all_arrays_finite": all(
            np.all(np.isfinite(value)) for value in snapshot.values()
        ),
        "base_ta_fields_are_160x160": all(
            snapshot[name].shape == (160, 160)
            for name in (
                "ta_temperature_rise_K",
                "ta_gradient_x_K_m",
                "ta_gradient_y_K_m",
            )
        ),
        "native_ta_field_is_320x320": snapshot[
            "ta_temperature_native_K"
        ].shape
        == (320, 320),
        "base_source_is_266x266": snapshot["source_power_xy_W"].shape
        == (266, 266),
        "base_temperature_nonnegative": bool(
            np.min(snapshot["ta_temperature_rise_K"]) >= 0.0
        ),
        "native_source_nonnegative": bool(
            np.min(snapshot["source_power_native_xy_W"]) >= 0.0
        ),
    }
    return snapshot, checks


def audit_case(
    case_root: Path,
    scenario_id: str,
    polarization: str,
    expected_report_sha256: str,
) -> tuple[dict[str, Any], dict[str, Any], dict[str, np.ndarray] | None]:
    supplied_root = case_root.expanduser()
    report_path = supplied_root.resolve() / scenario_id / polarization / REPORT_NAME
    report_exists = report_path.is_file()
    actual_report_sha = sha256(report_path) if report_exists else None
    report = (
        json.loads(report_path.read_text(encoding="utf-8"))
        if report_exists
        else {}
    )
    raw_record = report.get("raw", {})
    raw_supplied = Path(raw_record.get("path", "")).expanduser()
    raw_path = raw_supplied.resolve()
    raw_exists = raw_path.is_file()
    actual_raw_sha = sha256(raw_path) if raw_exists else None
    snapshot = None
    raw_error = None
    raw_checks: dict[str, bool] = {}
    if raw_exists:
        try:
            snapshot, raw_checks = _raw_snapshot(raw_path)
        except Exception as error:
            raw_error = repr(error)
    provenance = report.get("provenance", {})
    runner_supplied = Path(provenance.get("runner_path", "")).expanduser()
    runner_path = runner_supplied.resolve()
    runner_exists = runner_path.is_file()
    runner_sha = sha256(runner_path) if runner_exists else None
    expected_edges = thermal_edges(
        THERMAL_Z_REFINEMENT_FACTOR,
        xy_refinement_factor=THERMAL_XY_REFINEMENT_FACTOR,
        **SELECTED_DOMAIN,
    )
    expected_shape = [len(edge) - 1 for edge in expected_edges]
    mesh = report.get("thermal_mesh", {})
    expected_parameters = expected_model_parameters(scenario_id)
    checks = {
        "case_root_is_absolute": supplied_root.is_absolute(),
        "report_exists": report_exists,
        "expected_report_sha256_is_hex": len(expected_report_sha256) == 64
        and all(character in "0123456789abcdef" for character in expected_report_sha256),
        "report_sha256_matches": actual_report_sha == expected_report_sha256,
        "version_status_ready": report.get("version") == CASE_VERSION
        and report.get("status") == CASE_STATUS_READY
        and report.get("ready") is True,
        "scenario_identity_and_parameters_exact": report.get(
            "thermal_sensitivity_scenario"
        )
        == scenario_id
        and report.get("thermal_scenario_overrides")
        == SCENARIO_PARAMETERS[scenario_id]
        and report.get("thermal_model_parameters") == expected_parameters
        and report.get("polarization") == polarization,
        "selected_mesh_and_domain_exact": report.get("thermal_domain")
        == SELECTED_DOMAIN
        and report.get("thermal_xy_refinement_factor")
        == THERMAL_XY_REFINEMENT_FACTOR
        and report.get("thermal_z_refinement_factor")
        == THERMAL_Z_REFINEMENT_FACTOR
        and mesh.get("shape") == expected_shape
        and mesh.get("unknowns") == int(np.prod(expected_shape)),
        "diagnostic_scope_and_blocks_exact": report.get("diagnostic_only") is True
        and report.get("optical_mesh_blocked") is True
        and report.get("thermal_domain_size_converged") is True
        and report.get("thermal_scenario_sensitivity_characterized") is False
        and report.get("thermal_boundary_condition_uncertainty_converged") is False
        and report.get("thermal_interface_parameter_uncertainty_converged") is False
        and report.get("thermal_domain_and_boundary_converged") is False
        and report.get("production_mesh_selected") is False
        and report.get("optimizer_start_allowed") is False,
        "all_embedded_audits_ready": report.get(
            "prior_thermal_domain_certificate", {}
        ).get("ready")
        is True
        and report.get("input_audit", {}).get("ready") is True
        and report.get("raw_field_audit", {}).get("ready") is True
        and _all_true(report.get("mapping_checks", {}))
        and _all_true(report.get("mesh_checks", {}))
        and _all_true(report.get("solver_checks", {}))
        and _all_true(report.get("provenance_checks", {})),
        "raw_path_is_absolute": raw_supplied.is_absolute(),
        "raw_exists": raw_exists,
        "raw_sha256_matches_report": actual_raw_sha == raw_record.get("sha256"),
        "raw_snapshot_checks_all_true": _all_true(raw_checks),
        "native_array_shapes_match_selected_domain": snapshot is not None
        and snapshot["source_power_native_xy_W"].shape
        == tuple(expected_shape[:2])
        and snapshot["thermal_z_centers_m"].shape == (expected_shape[2],),
        "runner_path_is_absolute": runner_supplied.is_absolute(),
        "runner_exists": runner_exists,
        "runner_sha256_matches": runner_sha == provenance.get("runner_sha256"),
        "repository_was_clean": provenance.get(
            "repository_dirty_porcelain_before"
        )
        == ""
        and provenance.get("repository_dirty_porcelain_after") == "",
        "lumerical_was_not_used": provenance.get("lumerical_used") is False,
    }
    audit = {
        "scenario_id": scenario_id,
        "polarization": polarization,
        "report_path": str(report_path),
        "expected_report_sha256": expected_report_sha256,
        "actual_report_sha256": actual_report_sha,
        "raw_path": str(raw_path),
        "actual_raw_sha256": actual_raw_sha,
        "runner_path": str(runner_path),
        "runner_actual_sha256": runner_sha,
        "raw_error": raw_error,
        "checks": checks,
        "failed_checks": [name for name, passed in checks.items() if not passed],
        "ready": all(checks.values()),
    }
    return report, audit, snapshot if audit["ready"] else None


def compare_to_baseline(
    scenario_id: str,
    polarization: str,
    reports: Mapping[str, Mapping[str, Any]],
    snapshots: Mapping[str, Mapping[str, np.ndarray]],
) -> dict[str, Any]:
    baseline = snapshots["baseline"]
    scenario = snapshots[scenario_id]
    baseline_solution = reports["baseline"]["thermal_solution"]
    scenario_solution = reports[scenario_id]["thermal_solution"]
    gradient_numerator = np.sqrt(
        np.sum(
            (scenario["ta_gradient_x_K_m"] - baseline["ta_gradient_x_K_m"]) ** 2
        )
        + np.sum(
            (scenario["ta_gradient_y_K_m"] - baseline["ta_gradient_y_K_m"]) ** 2
        )
    )
    gradient_denominator = np.sqrt(
        np.sum(baseline["ta_gradient_x_K_m"] ** 2)
        + np.sum(baseline["ta_gradient_y_K_m"] ** 2)
    )
    source_nrmse = _nrmse(
        baseline["source_power_xy_W"], scenario["source_power_xy_W"]
    )
    integrity = {
        "base_coordinates_unchanged": np.allclose(
            baseline["ta_x_centers_m"],
            scenario["ta_x_centers_m"],
            rtol=0.0,
            atol=BASE_COORDINATE_ATOL_M,
        )
        and np.allclose(
            baseline["ta_y_centers_m"],
            scenario["ta_y_centers_m"],
            rtol=0.0,
            atol=BASE_COORDINATE_ATOL_M,
        ),
        "source_distribution_unchanged": source_nrmse <= SOURCE_NRMSE_LIMIT,
    }
    boundary = scenario_solution["solver"]["boundary_power_W"]
    total_boundary = max(sum(boundary.values()), np.finfo(float).tiny)
    return {
        "scenario_id": scenario_id,
        "polarization": polarization,
        "metrics": {
            "ta_temperature_map_nrmse": _nrmse(
                baseline["ta_temperature_rise_K"],
                scenario["ta_temperature_rise_K"],
            ),
            "ta_max_temperature_relative": _relative(
                float(baseline_solution["ta_base_max_temperature_rise_K"]),
                float(scenario_solution["ta_base_max_temperature_rise_K"]),
            ),
            "ta_mean_temperature_relative": _relative(
                float(baseline_solution["ta_base_mean_temperature_rise_K"]),
                float(scenario_solution["ta_base_mean_temperature_rise_K"]),
            ),
            "ta_combined_gradient_nrmse": float(
                gradient_numerator
                / max(float(gradient_denominator), np.finfo(float).tiny)
            ),
            "source_xy_power_nrmse": source_nrmse,
        },
        "boundary_power_W": boundary,
        "boundary_power_fraction": {
            name: float(value / total_boundary) for name, value in boundary.items()
        },
        "integrity_checks": integrity,
        "failed_integrity_checks": [
            name for name, passed in integrity.items() if not passed
        ],
        "integrity_pass": all(integrity.values()),
        "physical_acceptance_threshold_applied": False,
    }


def crosscheck_baseline_to_prior_domain(
    prior: Mapping[str, Any], snapshots: Mapping[str, Mapping[str, np.ndarray]]
) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for polarization in POLARIZATIONS:
        prior_audit = prior.get("case_audits", {}).get("combined_mid", {}).get(
            polarization, {}
        )
        raw_path = Path(prior_audit.get("raw_path", "")).expanduser().resolve()
        exists = raw_path.is_file()
        actual_sha = sha256(raw_path) if exists else None
        arrays_equal = {}
        if exists:
            with np.load(raw_path, allow_pickle=False) as archive:
                arrays_equal = {
                    name: np.array_equal(
                        np.asarray(archive[name]), snapshots[polarization][name]
                    )
                    for name in BASE_FIELD_NAMES
                }
        checks = {
            "prior_selected_domain_case_was_ready": prior_audit.get("ready") is True,
            "prior_raw_exists": exists,
            "prior_raw_sha256_revalidated": actual_sha
            == prior_audit.get("actual_raw_sha256"),
            "all_base_arrays_exactly_equal": bool(arrays_equal)
            and all(arrays_equal.values()),
        }
        result[polarization] = {
            "prior_raw_path": str(raw_path),
            "prior_raw_actual_sha256": actual_sha,
            "array_equal": arrays_equal,
            "checks": checks,
            "ready": all(checks.values()),
        }
    return result


def selection(characterized: bool, baseline_rebound: bool) -> dict[str, Any]:
    ready = characterized and baseline_rebound
    return {
        "baseline_exactly_rebound_to_selected_domain": baseline_rebound,
        "thermal_scenario_sensitivity_characterized": ready,
        "selected_diagnostic_frozen_q_thermal_z_factor": (
            THERMAL_Z_REFINEMENT_FACTOR if ready else None
        ),
        "selected_diagnostic_frozen_q_thermal_xy_factor": (
            THERMAL_XY_REFINEMENT_FACTOR if ready else None
        ),
        "selected_diagnostic_thermal_domain": SELECTED_DOMAIN if ready else None,
        "thermal_domain_size_converged": ready,
        "device_specific_physical_bounds_supplied": False,
        "thermal_boundary_condition_uncertainty_converged": False,
        "thermal_interface_parameter_uncertainty_converged": False,
        "thermal_material_parameter_uncertainty_converged": False,
        "thermal_domain_and_boundary_converged": False,
        "optical_mesh_converged": False,
        "electrical_mesh_converged": False,
        "production_multiphysics_mesh_selected": False,
        "optimizer_start_allowed": False,
    }


def build_certificate(
    prior_path: Path,
    prior_sha256: str,
    case_root: Path,
    report_sha256s: Mapping[str, Mapping[str, str]],
) -> dict[str, Any]:
    if set(report_sha256s) != set(SCENARIO_PARAMETERS) or any(
        set(report_sha256s[scenario_id]) != set(POLARIZATIONS)
        for scenario_id in SCENARIO_PARAMETERS
    ):
        raise ValueError("report SHA mapping must contain every scenario and Ea/Eb")
    prior, prior_audit = audit_prior_domain_certificate(prior_path, prior_sha256)
    reports: dict[str, dict[str, Any]] = {
        scenario_id: {} for scenario_id in SCENARIO_PARAMETERS
    }
    audits: dict[str, dict[str, Any]] = {
        scenario_id: {} for scenario_id in SCENARIO_PARAMETERS
    }
    snapshots: dict[str, dict[str, Any]] = {
        scenario_id: {} for scenario_id in SCENARIO_PARAMETERS
    }
    for scenario_id in SCENARIO_PARAMETERS:
        for polarization in POLARIZATIONS:
            report, audit, snapshot = audit_case(
                case_root,
                scenario_id,
                polarization,
                report_sha256s[scenario_id][polarization],
            )
            reports[scenario_id][polarization] = report
            audits[scenario_id][polarization] = audit
            snapshots[scenario_id][polarization] = snapshot
    all_cases_ready = all(
        audits[scenario_id][polarization]["ready"]
        for scenario_id in SCENARIO_PARAMETERS
        for polarization in POLARIZATIONS
    )
    comparisons: dict[str, Any] = {}
    if all_cases_ready:
        comparisons = {
            scenario_id: {
                polarization: compare_to_baseline(
                    scenario_id,
                    polarization,
                    {
                        name: reports[name][polarization]
                        for name in ("baseline", scenario_id)
                    },
                    {
                        name: snapshots[name][polarization]
                        for name in ("baseline", scenario_id)
                    },
                )
                for polarization in POLARIZATIONS
            }
            for scenario_id in SCENARIO_PARAMETERS
            if scenario_id != "baseline"
        }
    all_comparisons_integral = all_cases_ready and all(
        result["integrity_pass"]
        for scenario in comparisons.values()
        for result in scenario.values()
    )
    baseline = (
        crosscheck_baseline_to_prior_domain(prior, snapshots["baseline"])
        if all_cases_ready and prior_audit["ready"]
        else {}
    )
    baseline_ready = bool(baseline) and all(
        baseline[polarization]["ready"] for polarization in POLARIZATIONS
    )
    input_hashes = {
        reports[scenario_id][polarization]
        .get("input_audit", {})
        .get("actual_certificate_sha256")
        for scenario_id in SCENARIO_PARAMETERS
        for polarization in POLARIZATIONS
    }
    prior_hashes = {
        reports[scenario_id][polarization]
        .get("prior_thermal_domain_certificate", {})
        .get("actual_sha256")
        for scenario_id in SCENARIO_PARAMETERS
        for polarization in POLARIZATIONS
    }
    commits = {
        reports[scenario_id][polarization]
        .get("provenance", {})
        .get("repository_commit")
        for scenario_id in SCENARIO_PARAMETERS
        for polarization in POLARIZATIONS
    }
    global_checks = {
        "prior_thermal_domain_certificate_revalidated": prior_audit["ready"],
        "all_twenty_scenario_artifacts_revalidated": all_cases_ready,
        "baseline_exactly_rebounds_to_selected_domain": baseline_ready,
        "all_scenario_comparisons_preserve_source_and_coordinates": (
            all_comparisons_integral
        ),
        "all_cases_bind_one_optical_z32_certificate_hash": len(input_hashes) == 1
        and None not in input_hashes,
        "all_cases_bind_the_prior_domain_certificate_hash": prior_hashes
        == {prior_audit.get("actual_sha256")},
        "all_cases_bind_one_clean_runner_commit": len(commits) == 1
        and None not in commits,
        "production_and_optimizer_remain_forbidden": all(
            reports[scenario_id][polarization].get("production_mesh_selected")
            is False
            and reports[scenario_id][polarization].get("optimizer_start_allowed")
            is False
            for scenario_id in SCENARIO_PARAMETERS
            for polarization in POLARIZATIONS
        ),
    }
    ready = all(global_checks.values())
    return {
        "version": VERSION,
        "status": STATUS_READY if ready else STATUS_BLOCKED,
        "ready": ready,
        "scope": (
            "integrity-certified frozen-Q thermal boundary/interface/material "
            "scenario sensitivity; scenario endpoints are not confidence "
            "bounds and cannot certify an experimental device"
        ),
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "scenario_semantics": {
            "g_ta_sio2_7p37e4": (
                "named deposited-SiO2 estimate scenario; neither 7.37e4 nor "
                "7.37e6 is a unique device-specific measurement"
            ),
            "au_ta_1e6_to_perfect": (
                "1 and 100 MW/m2K are numerical scenarios; baseline 17.24 "
                "MW/m2K is an Au/MoS2 analogue, not Au/TaIrTe4 data"
            ),
            "ta_kz_0p5_to_2": (
                "numerical scenarios, not a sourced confidence interval"
            ),
            "far_xy_and_top_h": (
                "boundary-model brackets, not experimental boundary proof"
            ),
        },
        "prior_thermal_domain_certificate": prior_audit,
        "case_root": str(case_root.expanduser().resolve()),
        "case_audits": audits,
        "baseline_prior_domain_crosscheck": baseline,
        "comparisons_to_baseline": comparisons,
        "global_checks": global_checks,
        "failed_global_checks": [
            name for name, passed in global_checks.items() if not passed
        ],
        "selection": selection(all_comparisons_integral, baseline_ready),
        "production_multiphysics_mesh_selected": False,
        "optimizer_start_allowed": False,
    }


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--output-directory", type=Path, required=True)
    parser.add_argument("--prior-thermal-domain-certificate", type=Path, required=True)
    parser.add_argument("--prior-thermal-domain-certificate-sha256", required=True)
    parser.add_argument("--case-root", type=Path, required=True)
    for scenario_id in SCENARIO_PARAMETERS:
        for polarization in POLARIZATIONS:
            argument = scenario_id.replace("_", "-")
            parser.add_argument(
                f"--{argument}-{polarization.lower()}-report-sha256",
                required=True,
            )
    args = parser.parse_args()
    output = args.output_directory.expanduser().resolve()
    try:
        if not args.output_directory.expanduser().is_absolute():
            raise RuntimeError("output directory must be absolute")
        if not output.is_dir() or any(output.iterdir()):
            raise RuntimeError("output directory must be existing and empty")
        repository = Path(__file__).resolve().parents[3]
        dirty = _git(repository, "status", "--porcelain", "--untracked-files=all")
        if dirty != "":
            raise RuntimeError("repository must be clean before certification")
        hashes = {
            scenario_id: {
                polarization: getattr(
                    args, f"{scenario_id}_{polarization.lower()}_report_sha256"
                )
                for polarization in POLARIZATIONS
            }
            for scenario_id in SCENARIO_PARAMETERS
        }
        payload = build_certificate(
            args.prior_thermal_domain_certificate,
            args.prior_thermal_domain_certificate_sha256,
            args.case_root,
            hashes,
        )
        payload["certificate_provenance"] = {
            "repository_commit": _git(repository, "rev-parse", "HEAD"),
            "repository_dirty_porcelain": dirty,
            "generator_path": str(Path(__file__).resolve()),
            "generator_sha256": sha256(Path(__file__).resolve()),
        }
    except Exception as error:
        payload = {
            "version": VERSION,
            "status": STATUS_EXCEPTION,
            "ready": False,
            "error": repr(error),
            "traceback": traceback.format_exc(),
            "production_multiphysics_mesh_selected": False,
            "optimizer_start_allowed": False,
        }
    _atomic_json(output / CERTIFICATE_NAME, payload)
    print(
        json.dumps(
            {
                "certificate": str(output / CERTIFICATE_NAME),
                "status": payload["status"],
                "ready": payload["ready"],
                "selection": payload.get("selection"),
            }
        )
    )
    return 0 if payload["ready"] else 2


if __name__ == "__main__":
    raise SystemExit(main())
