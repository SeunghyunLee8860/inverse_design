#!/usr/bin/env python3
"""One frozen-Q thermal boundary or interface sensitivity scenario."""
...
from photothermal_pte.optimization_runs.au_dualpol_4um_current_switch.fdtdx_frozen_q_thermal_domain_certificate import (
    STATUS_READY as PRIOR_DOMAIN_STATUS_READY,
    VERSION as PRIOR_DOMAIN_VERSION,
)
...
VERSION = "fdtdx-frozen-q-thermal-sensitivity-case-v1"
STATUS_READY = "VALIDATED_DIAGNOSTIC_FDTDX_FROZEN_Q_THERMAL_SENSITIVITY_CASE"
STATUS_EXCEPTION = "BLOCKED_FDTDX_FROZEN_Q_THERMAL_SENSITIVITY_CASE_EXCEPTION"
REPORT_NAME = "FDTDX_FROZEN_Q_THERMAL_SENSITIVITY_CASE.json"
RAW_NAME = "FDTDX_FROZEN_Q_THERMAL_SENSITIVITY_CASE_FIELDS.npz"
POLARIZATIONS = ("Ea", "Eb")
...
SCENARIO_PARAMETERS = {
    ...
    "top_h0": {"top_air_convection_W_m2K": 0.0},
    "top_h20": {"top_air_convection_W_m2K": 20.0},
    "ta_sio2_7p37e4": {"g_ta_sio2_W_m2K": 7.37e4},
    "au_ta_1e6": {"g_au_ta_W_m2K": 1.0e6},
    "au_ta_100e6": {"g_au_ta_W_m2K": 100.0e6},
    "au_ta_perfect": {"g_au_ta_W_m2K": None},
    "ta_kz_0p5": {"ta_kappa_xyz_W_mK": [3.8, 14.4, 0.5]},
    "ta_kz_2": {"ta_kappa_xyz_W_mK": [3.8, 14.4, 2.0]},
}
...
def scenario_parameters(scenario_id: str) -> dict[str, object]:
    if scenario_id not in SCENARIO_PARAMETERS:
        raise ValueError(
            f"thermal sensitivity scenario must be one of "
            f"{tuple(SCENARIO_PARAMETERS)}"
        )
    value = SCENARIO_PARAMETERS[scenario_id]
    return {
...
def audit_prior_domain_certificate(
    ...
    checks = {
        ...
        "version_status_ready": payload.get("version") == PRIOR_DOMAIN_VERSION
        and payload.get("status") == PRIOR_DOMAIN_STATUS_READY
        ...
        "selected_mesh_and_domain_exact": selection.get(
        ...
        and selection.get("selected_diagnostic_thermal_domain")
        == SELECTED_DOMAIN
        and selection.get("thermal_domain_size_converged") is True,
        "boundary_and_production_remained_unselected": selection.get(
            "thermal_boundary_condition_uncertainty_converged"
        )
        is False
        and selection.get("thermal_domain_and_boundary_converged") is False
...
def run(
    ...
    prior_domain_certificate_path: Path,
    expected_prior_domain_certificate_sha256: str,
    ...
    scenario = scenario_parameters(scenario_id)
    ...
    _, prior_domain_audit = audit_prior_domain_certificate(
        prior_domain_certificate_path, expected_prior_domain_certificate_sha256
    )
    if not prior_domain_audit["ready"]:
        raise RuntimeError(
            f"prior thermal domain certificate failed: {prior_domain_audit}"
        )
    ...
    mesh_checks = {
        ...
        "scenario_overrides_applied_exactly": all(
            json.dumps(state.parameters[name], sort_keys=True)
            == json.dumps(expected, sort_keys=True)
            for name, expected in scenario.items()
        ),
        "declared_domain_bounds_exact": np.allclose(
            ...
            (
                -float(SELECTED_DOMAIN["lateral_half_span_um"]) * 1e-6,
                float(SELECTED_DOMAIN["lateral_half_span_um"]) * 1e-6,
                -float(SELECTED_DOMAIN["lateral_half_span_um"]) * 1e-6,
                float(SELECTED_DOMAIN["lateral_half_span_um"]) * 1e-6,
                -float(SELECTED_DOMAIN["substrate_depth_um"]) * 1e-6,
                float(SELECTED_DOMAIN["top_air_height_um"]) * 1e-6,
            ),
            rtol=0.0,
            atol=2e-18,
        ),
    ...
    provenance_checks = {
        ...
        "prior_domain_certificate_revalidated": prior_domain_audit["ready"] is True,
    ...
    payload: dict[str, Any] = {
        ...
        "ready": ready,
        "scope": (
            "diagnostic-only frozen-Q thermal boundary/interface/material "
            "scenario sensitivity at the selected mesh and domain; no "
            "confidence interval, optical/electrical/adjoint/optimizer, or "
            "production-mesh promotion"
        ),
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "polarization": polarization,
        "thermal_sensitivity_scenario": scenario_id,
        "thermal_scenario_overrides": scenario,
        "thermal_model_parameters": state.parameters,
        "thermal_domain": SELECTED_DOMAIN,
        "thermal_xy_refinement_factor": THERMAL_XY_REFINEMENT_FACTOR,
        "thermal_z_refinement_factor": THERMAL_Z_REFINEMENT_FACTOR,
        "prior_thermal_domain_certificate": prior_domain_audit,
        ...
        "thermal_domain_size_converged": True,
        "thermal_scenario_sensitivity_characterized": False,
...
def main() -> int:
    ...
    parser.add_argument(
        "--thermal-sensitivity-scenario",
        choices=tuple(SCENARIO_PARAMETERS),
        required=True,
    )
    ...
    try:
        payload = run(
            ...
            args.polarization,
            args.thermal_sensitivity_scenario,
        ...
        payload = {
            ...
            "thermal_sensitivity_scenario": args.thermal_sensitivity_scenario,
            "thermal_xy_refinement_factor": THERMAL_XY_REFINEMENT_FACTOR,
            "thermal_z_refinement_factor": THERMAL_Z_REFINEMENT_FACTOR,
            "diagnostic_only": True,
            "optical_mesh_blocked": True,
