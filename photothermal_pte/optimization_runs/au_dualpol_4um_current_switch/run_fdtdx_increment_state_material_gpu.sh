...
if [[ $# -lt 5 ]]; then
  echo "usage: $0 GPU_INDEX Ea|Eb /absolute/empty/output_directory /absolute/source_pair.json SOURCE_PAIR_SHA256 [runner options]" >&2
  exit 2
fi
...
if [[ ! -x "$python_bin" || ! -d "$source_dir" ]]; then
  echo "missing dedicated Python or patched FDTDX source" >&2
  exit 2
fi
...
if [[ "$source_pair" != /* || ! -f "$source_pair" ]]; then
  echo "source-pair certificate must be an existing absolute file" >&2
  exit 2
fi
if [[ ! "$source_pair_sha256" =~ ^[0-9a-f]{64}$ ]]; then
  echo "source-pair SHA256 must be 64 lowercase hexadecimal characters" >&2
  exit 2
fi
...
exec "$python_bin" -u \
  photothermal_pte/optimization_runs/au_dualpol_4um_current_switch/fdtdx_increment_state_exact_binary_mesh_case.py \
  --output-directory "$output_directory" \
  --source "$source_dir" \
  --source-pair "$source_pair" \
  --source-pair-sha256 "$source_pair_sha256" \
