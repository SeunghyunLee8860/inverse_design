...
from photothermal_pte.optimization_runs.au_dualpol_4um_current_switch.fdtdx_parity_reversible_microprobe import (
    ...
    reversible_source_audit,
)


def test_reversible_microprobe_source_audit_identifies_all_primitives() -> None:
    audit = reversible_source_audit()
    assert audit["status"] == "PASS"
    assert all(audit["checks"].values())
    assert len(audit["source_sha256"]) == 3


def test_reversible_connectivity_gate_passes_accurate_nonzero_gradient() -> None:
    ...
    assert status == "PASS_BOUNDED_REVERSIBLE_AD_CONNECTIVITY_ONLY"
    assert gates["directional_relative_error"] < MAX_DIRECTIONAL_RELATIVE_ERROR


def test_reversible_connectivity_gate_blocks_bad_or_slow_gradient() -> None:
    status, gates = reversible_connectivity_gate(
        value=1.0e-4,
