#!/usr/bin/env python3
"""Certify one common Ea/Eb normalization for increment-state source controls."""
...
VERSION = "fdtdx-increment-state-source-pair-v1"
STATUS_READY = "VALIDATED_FDTDX_INCREMENT_STATE_SOURCE_PAIR"
STATUS_BLOCKED = "BLOCKED_FDTDX_INCREMENT_STATE_SOURCE_PAIR"
...
def _expected_hash(value: str) -> bool:
...
def _load_case(
    ...
    actual_sha256 = _sha256(resolved) if exists else None
    ...
    checks = {
        ...
        "expected_sha256_is_hex": _expected_hash(expected_sha256),
        ...
        is True,
        "increment_state_exact": payload.get("dispersive_state_representation")
        == "increment",
        "per_case_scaling_unauthorized": payload.get(
            "per_case_scale_not_authorized_until_pair_comparison"
        )
        is True,
        "repository_clean": provenance.get("repository_dirty_porcelain") == "",
        "fdtdx_commit_exact": source.get("commit") == EXPECTED_FDTDX_COMMIT,
        "fdtdx_source_clean": source.get("dirty_porcelain") == "",
        "provenance_checks_all_true": bool(payload.get("provenance_checks"))
        and all(value is True for value in payload["provenance_checks"].values()),
...
def build_pair(
    ...
    numerical_case_identical = isinstance(
        numerical_case, dict
    ) and numerical_case == eb.get("numerical_case_contract")
    try:
        case_spec = (
            case_from_contract(numerical_case) if numerical_case_identical else None
        )
    ...
    cross_checks = {
        ...
        "polarization_vectors_exact": ea.get("source_contract", {}).get(
            "fixed_E_polarization_vector"
        )
        == [0.0, 1.0, 0.0]
        and eb.get("source_contract", {}).get("fixed_E_polarization_vector")
        == [1.0, 0.0, 0.0],
    ...
    return {
        ...
        "gates": cross_checks,
        "failed_gates": [name for name, passed in cross_checks.items() if not passed],
        ...
        "provenance": {
            ...
            "generator_sha256": _sha256(runner),
