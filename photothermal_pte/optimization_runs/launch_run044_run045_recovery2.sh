...
if [[ "$polarization" == "Ea" ]]; then
  ...
  initial=$raw/evaluation_0049_beta1.2_ansys_dfm_ld_mma_recovery1_latent.npz
  ...
  initial=$raw/evaluation_0040_beta1_ansys_dfm_ld_mma_recovery1_latent.npz
...
exec "$python_bin" -m "$module" \
  ...
  --recovery-append \
  --start-beta 1.2 \
  --output-slug ansys_dfm_ld_mma_recovery2
