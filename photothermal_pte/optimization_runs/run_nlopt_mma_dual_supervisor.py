#!/usr/bin/env python3
"""Run fresh NLopt LD_MMA Ea optimization, then Eb, after passed preflight."""
...
GPU = int(os.environ.get("TAIRTE4_NLOPT_MMA_GPU", "5"))
...
def write_status(status: str, **values: object) -> None:
    payload = {
        "schema": "nlopt-ld-mma-dual-supervisor-v1",
...
def main() -> int:
    ...
    for label, polarization, raw_root, published in RUNS:
        ...
        command = [
            sys.executable,
            "-m",
            "photothermal_pte.optimization_runs.tairte4_flake_topology.run_nlopt_mma_optimization",
        ...
        write_status("RUNNING_NLOPT_LD_MMA", run=label, polarization=polarization, command=command)
        completed = subprocess.run(command, cwd=REPOSITORY, env=environment)
        if completed.returncode or not passed(final):
            write_status(
                "BLOCKED_NLOPT_LD_MMA_RUN",
                run=label,
                polarization=polarization,
                returncode=completed.returncode,
            )
            raise RuntimeError(f"{label} NLopt LD_MMA run failed closed")
    write_status("VALIDATED_NLOPT_LD_MMA_EA_EB_OPTIMIZATIONS")
    return 0
