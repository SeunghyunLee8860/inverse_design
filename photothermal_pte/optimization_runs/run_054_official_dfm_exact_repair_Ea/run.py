#!/usr/bin/env python3
"""Launch fresh bounded Run054 on an explicitly selected GPU."""
...
RAW = ARTIFACT_ROOT / "run054_official_dfm_exact_repair_Ea_evaporated_v1"
...
def main() -> int:
    gpu = int(os.environ.get("RUN054_GPU", "1"))
    ...
    environment.update(
        {
            ...
            "XDG_CONFIG_HOME": f"/tmp/seunghyun_lumerical_run054_gpu{gpu}",
            "MPLCONFIGDIR": f"/tmp/seunghyun_matplotlib_run054_gpu{gpu}",
        }
    )
    command = [
        sys.executable,
