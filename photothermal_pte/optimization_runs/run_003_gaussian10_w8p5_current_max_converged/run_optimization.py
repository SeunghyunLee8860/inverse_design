#!/usr/bin/env python3
"""Restartable convergence-based constrained optimization for Run 003."""
...
def checkpoint_git(tag: str, extra: list[Path] | None = None) -> None:
    ...
    if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=REPOSITORY).returncode:
        execute(["git", "commit", "-m", f"Record Run 003 {tag}"], f"git_commit_{tag}")
...
def propose(
    ...
    payload = {
        "status": "PROPOSED_RUN003_FIXED_INEQUALITY_MMA_STEP", "passed": True,
...
def finalize(current_raw: Path, beta: float, global_iteration: int, gpu: str) -> None:
    ...
    (HERE / "results/OPTIMIZATION_REPORT.md").write_text(
        "# Run 003 fully binarized constrained PTE optimization\n\n"
...
def main() -> int:
    ...
    for beta_integer in BETA_SCHEDULE:
        ...
        if beta >= 64.0 and projected_binary_gate(current_metrics):
            ...
            return 0
    raise RuntimeError("projected binary and exact 500 nm gates did not pass through beta=4096")
