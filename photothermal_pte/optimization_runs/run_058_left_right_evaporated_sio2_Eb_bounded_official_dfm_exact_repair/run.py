#!/usr/bin/env python3
"""Launch fresh left/right-contact evaporated-SiO2 E||b Run058."""
...
RAW = ARTIFACT_ROOT / "run058_bounded_official_dfm_exact_repair_Eb_evaporated_v1"
...
def main() -> int:
    gpu = int(os.environ.get("RUN058_GPU", "0"))
    ...
    environment.update(
        {
            ...
            "LUMERICAL_GPU_ENGINE_LOCK": f"/tmp/seunghyun_lumerical_run058_gpu{gpu}.lock",
            "XDG_CONFIG_HOME": f"/tmp/seunghyun_lumerical_run058_gpu{gpu}",
            "MPLCONFIGDIR": f"/tmp/seunghyun_matplotlib_run058_gpu{gpu}",
        }
    )
    command = [
        sys.executable,
    ...
    if os.environ.get("RUN058_RESUME", "0") == "1":
