#!/usr/bin/env python3
"""Launch fresh +45-degree-contact evaporated-SiO2 E||b Run060."""
...
RAW = ARTIFACT_ROOT / "run060_bounded_official_dfm_exact_repair_Eb_evaporated_v1"
BASE_ROOT = Path(
...
def main() -> int:
    ...
    environment.update(
        {
            ...
            "LUMERICAL_GPU_ENGINE_LOCK": f"/tmp/seunghyun_lumerical_run060_gpu{gpu}.lock",
            "XDG_CONFIG_HOME": f"/tmp/seunghyun_lumerical_run060_gpu{gpu}",
            "MPLCONFIGDIR": f"/tmp/seunghyun_matplotlib_run060_gpu{gpu}",
        }
    )
    command = [
        sys.executable,
    ...
    if os.environ.get("RUN060_RESUME", "0") == "1":
