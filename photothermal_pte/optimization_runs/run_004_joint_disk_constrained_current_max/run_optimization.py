#!/usr/bin/env python3
"""Run 004: joint-FOM/disk-constrained continuation from the original state."""
...
RUN_RAW = Path(
    os.environ.get(
        "RUN004_RAW_ROOT",
        "/data/seunghyun/tairte4/raw_artifacts/run004_joint_disk_constrained_20260808",
    )
).expanduser().resolve()
...
BETA_SCHEDULE = [2, 4, 8, 16, 32, 64, 128, 256, 512]
...
def checkpoint_git(tag: str, extra: list[Path] | None = None) -> None:
    ...
    if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=REPOSITORY).returncode:
        execute(["git", "commit", "-m", f"Record Run 004 {tag}"], f"git_commit_{tag}")
...
def propose(
    ...
    payload = {
        "status": "PROPOSED_RUN004_JOINT_DISK_CONSTRAINED_MMA_STEP", "passed": True,
...
def finalize(current_raw: Path, beta: float, global_iteration: int, gpu: str) -> None:
    ...
    (HERE / "results/OPTIMIZATION_REPORT.md").write_text(
        "# Run 004 fully binarized constrained PTE optimization\n\n"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="pause cleanly after this many newly accepted updates; 0 runs full continuation",
    )
    args = parser.parse_args()
    if args.pilot_accepted_updates < 0:
        raise ValueError("--pilot-accepted-updates must be nonnegative")
    os.environ["RUN004_CONSTRAINT_TORCH_DEVICE"] = args.constraint_device
    verify_inputs()
    emit(
        "driver_started",
        gpu=args.gpu,
        constraint_device=args.constraint_device,
        method="joint FOM and 500 nm disk-constraint continuation",
    )
    ...
    for beta_integer in BETA_SCHEDULE:
            ...
            for retry, move in enumerate(move_retries):
                ...
                if decision["accepted"]:
                    ...
                    if (
                        ...
                        pause_status = "PAUSED_AFTER_RUN004_GPU_PILOT"
                        summary_path = HERE / "results/optimization_summary.json"
        ...
        if beta >= 64.0 and projected_binary_gate(current_metrics):
            ...
            return 0
    raise RuntimeError("projected binary and exact 500 nm gates did not pass through beta=512")
