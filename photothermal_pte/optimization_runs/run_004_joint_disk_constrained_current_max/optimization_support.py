"""Joint objective/manufacturability support for Run 004."""
...
def _constraint_device() -> torch.device:
    requested = os.environ.get("RUN004_CONSTRAINT_TORCH_DEVICE", "cpu")
...
def stage_caps(beta: float) -> np.ndarray:
    if beta <= 2.0:
        values = (1.25e-3, 3.00e-5)
    elif beta <= 4.0:
        values = (1.00e-3, 2.50e-5)
    elif beta <= 8.0:
        values = (7.50e-4, 2.00e-5)
    elif beta <= 16.0:
        values = (5.00e-4, 1.50e-5)
    elif beta <= 32.0:
        values = (2.50e-4, 1.00e-5)
    elif beta <= 64.0:
        values = (1.00e-4, 7.50e-6)
    elif beta <= 128.0:
        values = (5.00e-5, 5.00e-6)
    elif beta <= 256.0:
        values = (2.50e-5, 2.50e-6)
    elif beta <= 512.0:
        values = (1.50e-5, 1.50e-6)
    else:
        values = (1.00e-5, 1.00e-6)
...
def mma_effective_constraint_caps(
    ...
    """Limit phase degradation without freezing topology changes.

    Run 003 used all available slack in one phase to repair the other and later
    switched to a zero-slack phase-preserving cap.  The latter can freeze
    threshold events.  Run 004 instead allows only one percent relative (or
    ``1e-7`` absolute) local slack while retaining immutable stage caps.
    """
...
def candidate_acceptance(
    ...
    if current_feasible:
        accepted = candidate_feasible and objective_ratio >= 0.995
        reason = "remain jointly feasible and limit actual FOM loss to 0.5%"
    else:
        violation_reduction = (current_v - candidate_v) / max(current_v, 1.0e-300)
        accepted = (
            (candidate_feasible or violation_reduction >= 0.01)
            and objective_ratio >= 0.98
        )
        reason = "reduce fixed-cap violation by 1% while limiting FOM loss to 2%"
    exact_gate_enabled = (
