...
def get_reference_layer(#-- Input parameters ---------------------|
    ...
    periodic_axes=None, # e.g. [0,1]: wrap MFS/MGS filtering at the unit-cell
                        # boundaries (periodic metasurface); None -> legacy
                        # Air-pad + forced-air border behavior
):                                                                #-- Impose MFS and MGS on x ------------------------|
    Is_periodic = periodic_axes is not None                             #                                             |
    ...
    #-----------------------------------------------------------------------------------------------------------------|
    if Is_periodic:                               # Periodic unit cell: opposite edges are neighbors ------------------|
        Air_pad= 0                                # -> no Air pad, no forced-air border; filters wrap via              |
        print("Periodic MFS/MGS filtering (no Air pad / no border mask)")   # Filters periodic_axes support           |
    elif Mask_pixels == 0:                                                                   #                        |
        if Is_waveguide:                                                                     #                        |
    ...
    x_ref = x_ref.flatten()                                                                  #                        |
    if Is_periodic:                                                                          # Periodic: NO forced-air|
        pass                                                                                 #   border mask          |
    elif Is_waveguide:                                                                       # Masking            ----|
    ...
    while Dilation_index < N_dilation:    #------- Activate multiple dilation N_dilation times -----------------------|
        x_copy= Ft.conic_filter(npa.clip(x_copy, 0.0, 1.0), Dilation_R, DR_width, DR_length, DR_res, periodic_axes=periodic_axes)    #             |
        ...
        if Smoothing_D and Dilation_index == round(N_dilation/2)-1: # MGS < MFS, Smoothing when MFS = MGS ------------|
            x_copy= Ft.conic_filter(npa.clip(x_copy, 0.0, 1.0), Half_R, DR_width, DR_length, DR_res, periodic_axes=periodic_axes)              #   |
    ...
    while Erosion_index < N_erosion:      #--------- Activate multiple erosion N_erosion times -----------------------|
        if Smoothing_E:                             # MGS >= MFS -----------------------------------------------------|
            ...
            if Erosion_index == N_Half:              # Smoothing when MFS = MGS --------------------------------------|
                ...
                x_copy= Ft.conic_filter(npa.clip(x_copy, 0.0, 1.0), Half_R, DR_width, DR_length, DR_res, periodic_axes=periodic_axes)          #   |
                x_copy = tanh_projection_m(x_copy, beta, 0.5)                             # Compensation smoothing    |
        x_copy= Ft.conic_filter(npa.clip(x_copy, 0.0, 1.0), Erosion_R, DR_width, DR_length, DR_res, periodic_axes=periodic_axes)               #   |
        x_copy = tanh_projection_m(x_copy, beta, 1-eta_Ref)                               # Unit erosion              |
        Erosion_index +=1                                                                                         #   |
    if Air_pad > 0:                                                                                               #   |
